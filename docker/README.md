...
The docker image will run matrix-trivia-bot with a SQLite database and
...
* Choose whether you want to use SQLite or Postgres as your database backend.
  ...
  ```
  database: "postgres://username:password@postgres/matrix-trivia-bot?sslmode=disable"
  ```
...
```
docker-compose up matrix-trivia-bot
```
...
```
docker-compose up -d matrix-trivia-bot
```

(Logs can later be accessed with the `docker logs` command).

This will use the `latest` tag from
[Docker Hub](https://hub.docker.com/somebody/matrix-trivia-bot).
...
```
docker-compose pull matrix-trivia-bot
```
...
[matrix-trivia-bot.service](matrix-trivia-bot.service). The service uses
`docker-compose` to start and stop the bot.

Copy the file to `/etc/systemd/system/matrix-trivia-bot.service` and edit to
match your setup. You can then start the bot with:

```
systemctl start matrix-trivia-bot
```

and stop it with:

```
systemctl stop matrix-trivia-bot
```

To run the bot on system startup:

```
systemctl enable matrix-trivia-bot
```
...
```
docker build -t somebody/matrix-trivia-bot:latest -f docker/Dockerfile .
```
